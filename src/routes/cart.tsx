...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Dein OLD IRON Warenkorb. Sichere Zahlung, kostenloser Versand ab 99 €." },
    ],
  }),
  component: CartPage,
});
...
function CartPage() {
  ...
  const freeShippingEligible = items.length > 0 && subtotal >= FREE_SHIPPING_THRESHOLD;
  const shipping = items.length === 0 || freeShippingEligible ? 0 : SHIPPING_COST;
  const total = subtotal + shipping;
  // Inklusive MwSt-Berechnung (Bruttopreise sind inkl. MwSt.)
  ...
  const checkout = async () => {
    ...
    try {
      ...
      if (!profile?.shipping_address || !profile.shipping_city || !profile.shipping_zip) {
        toast.error("Bitte hinterlege zuerst eine Lieferadresse.");
        navigate({ to: "/account" });
        return;
      }
      ...
      const msg = e instanceof Error ? e.message : "Bestellung konnte nicht angelegt werden.";
      toast.error(msg);
    } finally {
      setPlacing(false);
    }
    ...
    <div className="bg-background text-foreground min-h-screen">
      ...
      <main className="pt-stack-lg pb-stack-lg px-margin-mobile md:px-margin-desktop max-w-[1440px] mx-auto">
        <header className="mb-stack-md pt-8">
          <h1 className="font-display text-[48px] md:text-[56px] uppercase tracking-tight text-primary leading-none">Warenkorb</h1>
          <p className="text-[13px] font-semibold uppercase tracking-widest text-outline mt-2">Präzision in jeder Naht.</p>
        </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-stack-md">
                    ...
                    <div>
                      ...
                      <div className="flex gap-4 mt-4 items-center flex-wrap">
                        ...
                        <div className="flex items-center gap-3 border border-outline-variant px-3 py-1">
                          <button onClick={() => updateQty(item.id, -1)} className="text-outline hover:text-primary cursor-pointer" aria-label="Menge verringern">−</button>
                          <span className="text-[14px] font-semibold w-4 text-center">{item.qty}</span>
                          <button onClick={() => updateQty(item.id, 1)} className="text-outline hover:text-primary cursor-pointer" aria-label="Menge erhöhen">+</button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-4 space-y-gutter">
              <div className="bg-surface-container p-gutter space-y-6 border border-outline-variant/30">
                <h2 className="font-headline text-[22px] uppercase text-primary">Bestellübersicht</h2>
                <div className="space-y-3 text-[15px] text-secondary border-b border-outline-variant/30 pb-6">
                  ...
                  <div className="flex justify-between">
                    <span>Versand</span>
                    <span>{freeShippingEligible ? "Kostenlos" : `${SHIPPING_COST.toFixed(2)} €`}</span>
                  </div>
                  {!freeShippingEligible && (
                    <div className="bg-accent-warm/10 border border-accent-warm/30 px-3 py-2 text-[12px] text-secondary leading-snug">
                      <span className="text-accent-warm font-semibold">Nur noch {(FREE_SHIPPING_THRESHOLD - subtotal).toFixed(2)} € bis zum kostenlosen Versand!</span>
                    </div>
                  )}
                  ...
                  <p className="text-[11px] text-outline italic">inkl. 19 % MwSt. ({vat.toFixed(2)} €)</p>
                </div>
                <div className="space-y-4 pt-2">
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <span className="text-[12px] text-outline leading-tight">
                      Ich akzeptiere die{" "}
                      <Link to="/legal/agb" className="underline text-secondary hover:text-primary">AGB</Link>,{" "}
                      <Link to="/legal/datenschutz" className="underline text-secondary hover:text-primary">Datenschutzerklärung</Link> und{" "}
                      <Link to="/legal/widerruf" className="underline text-secondary hover:text-primary">Widerrufsbelehrung</Link>.
                    </span>
                  </label>
                  <button onClick={checkout} disabled={!terms || placing}
                    ...
                    {placing ? "Wird verarbeitet..." : "Zur sicheren Zahlung"}
                  </button>
                </div>
              </div>
              ...
              <div className="bg-surface-container-low p-gutter grid grid-cols-2 gap-4 border border-outline-variant/20">
                {[
                  ...
                  ["local_shipping",   "Versand aus DE"],
                  ["workspace_premium","Premium Qualität"],
                  ["keyboard_return",  "14 Tage Widerruf"],
                ].map(([icon, label]) => (
